...
async function updateVote(userId, vote) {
  ...
  if (!previousVote || previousVote !== vote) {
    ...
    // Salvar no MongoDB
    await VoteHistory.create({ userId, name: user.name, city: user.city, vote });
  }
}
...
async function broadcastUpdates() {
  ...
  wss.clients.forEach(client => {
    if (client.readyState === 1) {
      client.send(JSON.stringify({ type: 'update', votes, lastVotes, votingClosed }));
    }
  });
...
wss.on('connection', async (ws) => {
  ...
  ws.on('message', async (message) => {
    ...
    try {
      ...
      if (!userId) {
        ws.send(JSON.stringify({ type: 'error', message: 'É necessário um userId' }));
        return;
      }
...
// Middleware para verificar se o usuário está tentando votar com muita frequência
app.use(async (req, res, next) => {
  ...
  if (lastVoteTime && currentTime - lastVoteTime < cooldown) {
    const remainingTime = ((cooldown - (currentTime - lastVoteTime)) / 1000).toFixed(1);
    return res.status(500).json({ error: `Erro voce não pode votar con tanta frequencia, deve esperar mais ${remainingTime} segundos` });
  }
...
// Middleware para verificar se o usuário está repetindo o mesmo voto
app.use(async (req, res, next) => {
  const { userId, vote } = req.body;
  if (!userId || !vote) return next();

  const previousVote = await redis.get(`user:${userId}`);
  if (previousVote === vote) {
    return res.status(500).json({ error: 'Erro ao usuario repetiu o mesmo voto' });
  }
